...
public class ChessMatch {
	...
	public ChessPiece[][] getPieces() {
		...
		for (int i = 0; i < board.getRows(); i++) {
			for (int j = 0; j < board.getColumns(); j++) {
				mat[i][j] = (ChessPiece) board.piece(i, j); // downcast para interpretar a peça como peça do tipo
															// ChessPiece ao inves de Piece
	...
	private Piece makeMove(Position source, Position target) {
		...
		// JOGADA ESPECIAL ROQUE DO LADO DO REI 
		...
		// JOGADA ESPECIAL ROQUE DO LADO DA RAINHA
		if(p instanceof King && target.getColumn() == source.getColumn() - 2) {
	...
	private void validadeTargetPosition(Position source, Position target) {
		...
		if (!board.piece(source).possibleMove(target)) {
			throw new ChessException("A peça de origem não pode se mover para posição de destino.");
		}
